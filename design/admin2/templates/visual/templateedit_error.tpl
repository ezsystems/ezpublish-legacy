...
<h2><span class="time">[{currentdate()|l10n( shortdatetime )}]</span> {'The template cannot be edited.'|i18n( 'design/admin/visual/templateedit' )}</h2>
...
{if $original_template}
<div class="block">
<label>{'Override template'|i18n( 'design/admin/visual/templateedit' )}:</label>
...
<div class="block">
<input {if or( not( $template_exists ), not( $is_readable ) )}class="button-disabled" disabled="disabled"{else}class="button"{/if} type="submit" name="OpenReadOnly" value="{'Open as read only'|i18n( 'design/admin/visual/templateedit' )}" />
