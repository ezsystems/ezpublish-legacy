    ...
    function _searchCallBack( data )
    {
        if ( data && data.content !== '' )
        {
            ...
            if ( data.content.SearchResultCount )
            {
                ...
                {
                    html += '<a onclick="return ezajaxrelationsSearchAddObject( this, \'' + boxID + '\', ' + arr[i].id + ',\'' + $('</div>').text(arr[i].name).html() + '\',\'' + arr[i].class_name + '\',\'' + arr[i].section.name + '\',\'' + pub.val() + '\'  );" title="' + arr[i].path_identification_string + '">' + arr[i].name + '<\/a><br \/>';
                }
