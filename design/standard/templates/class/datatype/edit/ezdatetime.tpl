...
<div class="block">
	...
	<select name="ContentClass_ezdatetime_default_{$class_attribute.id}">
	    ...
	    <option value="1" {section show=eq($class_attribute.data_int1,1)}selected{/section}>{"Current datetime"|i18n("design/standard/class/datatype")}</option>
            <option value="2" {section show=eq($class_attribute.data_int1,2)}selected{/section}>{"Custom value"|i18n("design/standard/class/datatype")}</option>
	</select>
    </div>

    <div class="element">
        <label>{"Custom default value"|i18n("design/standard/class/datatype")}</label><div class="labelbreak"></div>
        <table border="0">
	<tr>
	  <td>
