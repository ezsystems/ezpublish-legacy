...
<select name="ContentClass_ezpackage_view_mode_{$class_attribute.id}" size="1">
      <option value="0" {section show=eq($class_attribute.data_int1,0)}selected{/section}>combobox</option>
      <option value="1" {section show=eq($class_attribute.data_int1,1)}selected{/section}>icon view</option>
</select>
</div>