...
<tr>
  <th align="left"><a href={concat($module.functions.grouplist.uri,"/id")|ezurl}>{"ID"|i18n('content/class')}</a></th>
  <th align="left"><a href={concat($module.functions.grouplist.uri,"/name")|ezurl}>{"Name"|i18n('content/class')}</a></th>
  <th align="left"><a href={concat($module.functions.grouplist.uri,"/creator")|ezurl}>{"Creator"|i18n('content/class')}</a></th>
  <th align="left"><a href={concat($module.functions.grouplist.uri,"/modifier")|ezurl}>{"Modifier"|i18n('content/class')}</a></th>
  <th align="left"><a href={concat($module.functions.grouplist.uri,"/created")|ezurl}>{"Created"|i18n('content/class')}</a></th>
  <th align="left"><a href={concat($module.functions.grouplist.uri,"/modified")|ezurl}>{"Modified"|i18n('content/class')}</a></th>
</tr>

{section name=Groups loop=$groups sequence=array(bglight,bgdark)}
<tr>
  <td class="{$Groups:sequence}" width="3%">{$Groups:item.id}</td>
  <td class="{$Groups:sequence}"><a href={concat($module.functions.classlist.uri,"/",$Groups:item.id)|ezurl}>{$Groups:item.name}</a></td>
