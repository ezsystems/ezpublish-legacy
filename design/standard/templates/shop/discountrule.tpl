...
<tr>
    ...
    <th>{"Discount Percent"|i18n('content/class')}:</th>
</tr>
{section name=Rules loop=$discountrule_array sequence=array(bglight,bgdark)}
<tr>
    <td class="{$Rules:sequence}" width="1%">{$Rules:item.id}</td>
    ...
    <td class="{$Rules:sequence}">{$Rules:item.discount_percent}%</td>
...
{include uri="design:gui/button.tpl" name=newrule id_name=AddDiscountRuleButton value="New Rule"|i18n('shop')}
