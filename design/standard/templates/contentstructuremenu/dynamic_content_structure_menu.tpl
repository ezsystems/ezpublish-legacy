<script language="JavaScript" type="text/javascript">
    var ODtextStrings=new Array();
    ODtextStrings["PleaseWait"]="{'Please wait...'|i18n('design/standard/contentstructuremenu/dynamic')}";
</script>
...
{let rootNodeID             = ezini( 'TreeMenu', 'RootNodeID'       , 'contentstructuremenu.ini' )
    ...
    {*cache-block keys=array($:itemClickAction, $:rootNodeID,$current_user.role_id_list|implode( ',' )) subtree_expiry=$:rootNode.url_alias*}
        ...
        {* Show menu tree. All container nodes are unfolded. *}
            <ul id="{$:menuID}">
                {include uri="design:contentstructuremenu/show_dynamic_content_structure.tpl" contentStructureTree=$contentStructureTree class_icons_size=$:classIconsSize csm_menu_item_click_action=$:itemClickAction ui_context=$ui_context is_root_node=true() nodesList=$nodesList lastNodeID=$rootNodeID}
            </ul>
    {*/cache-block*}
    {/default}
