...
{section name=Result loop=$DeleteResult}
    <li>{"Removing %1 will remove the node itself and it's %2!"|i18n("design/standard/node",,hash("%1",$Result:item.nodeName,"%2",$Result:item.childCount))} {$Result:item.additionalWarning}</li>
{/section}
</ul>
</div>
