...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        <img src={$image.url|ezroot} width="{$image.width}" height="{$image.height}" {if $hspace}hspace="{$hspace}"{/if} style="{$inline_style}" alt="{$alt_text|wash(xhtml)}"{if not(and( is_set( $link_title ), $link_title ))} title="{$title}"{/if} />
        {if $href}</a>{/if}
    {/if}
