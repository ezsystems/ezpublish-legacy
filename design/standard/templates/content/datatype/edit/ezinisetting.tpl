...
{* String. *}
{case in=array( 1, 4, 5 )}
    <input class="box" type="text" size="50" name="{$attribute_base}_ini_setting_{$attribute.id}" value="{$attribute.data_text|wash}" />
{/case}
...
{case match=6}
    <div class="block">
        ...
        <input type="checkbox" name="{$attribute_base}_ini_setting_make_empty_array_{$attribute.id}" {$attribute.data_int|choose( '', 'checked=checked' )} />
    </div>
