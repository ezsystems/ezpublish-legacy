...
{if count( $attribute.content.modified )}
...
{foreach $attribute.content.modified as $modified}
<ul>
<li>{'Ini File: '|i18n( 'design/standard/class/datatype' )}{$modified.file|wash}</li>
<li>{'Ini Value: '|i18n( 'design/standard/class/datatype' )}{$modified.ini_value|wash}</li>
</ul>
{/foreach}
</div>
{/if}
