...
{if $version_list}
    <table class="list" width="100%" cellspacing="0" cellpadding="0" border="0">
        ...
        <tr class="{$seq}">
            <td>
                <a href={concat( "/content/versionview/", $object.id, "/", $version.version, "/", $edit_language|not|choose( concat( $edit_language,"/" ), "" ) )|ezurl}>{$version.version}</a>
                {if eq($version.version,$object.current_version)}*{/if}
            </td>
            <td>
                {$version.status|choose("Draft","Published","Pending","Archived","Rejected")}
            </td>
            <td>
                {foreach $version.language_list as $lang}
                    {delimiter}<br />{/delimiter}
                    <img src="{$lang.language_code|flag_icon}" alt="{$lang.language_code}" />&nbsp;
                    <a href={concat("/content/versionview/",$object.id,"/",$version.version,"/",$lang.language_code,"/")|ezurl}>{$lang.locale.intl_language_name}</a>
                {/foreach}
            </td>
            <td>
...
<form action={concat( $module.functions.diff.uri, '/', $object.id, '/' )|ezurl} method="post">
        <select name="FromVersion">
            {foreach $object.versions as $ver}
                <option {if eq( $ver.version, $selectOldVersion)}selected="selected"{/if} value={$ver.version}>{$ver.name|shorten(35)} [{$ver.version}]</option>
            {/foreach}
        </select>
        <select name="ToVersion">
            {foreach $object.versions as $ver}
                <option {if eq( $ver.version, $selectNewVersion)}selected="selected"{/if} value={$ver.version}>{$ver.name|shorten(35)} [{$ver.version}]</option>
            {/foreach}
        </select>
...
{literal}
<script type="text/javascript">
document.write('<div class="context-toolbar"><div class="block"><ul><li><a href="#" onclick="show(\'diffview\', \'previous\'); return false;">Old values only</a></li><li><a href="#" onclick="show(\'diffview\', \'inlinechanges\'); return false;">Inline changes</a></li><li><a href="#" onclick="show(\'diffview\', \'blockchanges\'); return false;">Block changes</a></li><li><a href="#" onclick="show(\'diffview\', \'latest\'); return false;">New values only</a></li></ul></div></div>');
</script>
{/literal}
