...
{section name=File loop=$file_results.result_elements}{section-exclude match=$:item.result}chmod a+rwx {$:item.file}<br/>{/section}</pre>
...
<h3>Example</h3>
<pre class="example">chmod og+rwx var
chown nouser.nouser var</pre>
