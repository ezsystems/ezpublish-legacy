...
{section show=eq( $select_siteaccess, true )}
    ...
    <div class="block">
    <label>{'Siteaccess:'|i18n( 'design/standard/pagelayout' )}</label>
    <select name="siteaccesslist"{eq( $ui_context, 'edit' )|choose( '', ' disabled="disabled"' )} onchange='location.href=this.value'>
            <option value={'/user/preferences/set/admin_quicksettings_siteaccess/global_override'|ezurl}{if eq( $siteaccess, 'global_override')} selected="selected"{/if}>{'Global (override)'|i18n( 'design/standard/pagelayout' )}</option>
...
{/section}
<div class="block">
<input {eq( $ui_context, 'edit' )|choose( "class='button'", "class='button-disabled'" )}{eq( $ui_context, 'edit' )|choose( '', ' disabled="disabled"' )} type="submit" name="SetButton" value="{'Set'|i18n( 'design/standard/pagelayout' )}" />
</div>
