...
{section show=eq( $select_siteaccess, true )}
    ...
    {section loop=$siteaccesslist}
    		<option onclick='location.href={concat( '/user/preferences/set/admin_quicksettings_siteaccess/', $:item )|ezurl}'{section show=eq( $siteaccess, $:item )} selected="selected"{/section}>{$:item|wash}</option>
    {/section}
    </select>
    </div>
    {/let}
