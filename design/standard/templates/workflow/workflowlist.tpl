...
<tr>
    <td class="{$WorkflowSequence:item}">{$Workflow:item.id}</td>
    ...
    <td class="{$WorkflowSequence:item}"><span class="small">{$Workflow:item.created|l10n(shortdatetime)}</span></td>
...
{include uri="design:gui/button.tpl" name=new id_name=NewWorkflowButton value="New Workflow"}
