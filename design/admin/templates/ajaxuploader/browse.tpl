...
<h4>
    {if $browse_start.node_id|ne( 1 )}
        <a href="ezajaxuploader::browse::{$browse_start.parent_node_id}::{$class.id}"><img src={'up-16x16-grey.png'|ezimage()} alt="{'Go to the parent level'|i18n( 'design/admin/ajaxuploader' )}" /></a>
    {/if}
...
<table class="list ajaxuploader-locations">
    <tr>
        <th class="tight"></th>
        <th>{'Name'|i18n( 'design/admin/ajaxuploader' )}</th>
        <th>{'Type'|i18n( 'design/admin/ajaxuploader' )}</th>
        <th>{'Section'|i18n( 'design/admin/ajaxuploader' )}</th>
    </tr>
...
<div class="block pagination">
    <div class="left" style="text-align:right;">
        {if $browse.has_prev}
            <a href="ezajaxuploader::browse::{$browse_start.node_id}::{$class.id}::{$browse.offset|sub( $browse.limit )}">&laquo;&nbsp;{'prev'|i18n( 'design/admin/ajaxuploader' )}</a>
        {else}
            <span>&laquo;&nbsp;{'prev'|i18n( 'design/admin/ajaxuploader' )}</span>
        {/if}
    </div>
    <div class="right">
        {if $browse.has_next}
            <a href="ezajaxuploader::browse::{$browse_start.node_id}::{$class.id}::{$browse.offset|sum( $browse.limit )}">{'next'|i18n( 'design/admin/ajaxuploader' )}&nbsp;&raquo;</a>
        {else}
            <span>{'next'|i18n( 'design/admin/ajaxuploader' )}&nbsp;&raquo;</span>
        {/if}
    </div>
    <div class="break"></div>
</div>
