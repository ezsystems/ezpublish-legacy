...
<h2 class="context-title"><a href={$node.depth|gt(1)|choose('/'|ezurl,$node.parent.url_alias|ezurl)}18n( 'design/standard/node/view' )}" title="{'Up one level'|i18n( 'design/standard/node/view' )}" /></a>&nbsp;{'Items inside'|i18n( 'design/admin/node/view/full' )}&nbsp;<i>{$node.name|wash}</i>&nbsp;[{$children_count}]</h2>
