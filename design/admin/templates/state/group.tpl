...
<div class="block">
  {if $state_group.is_internal|not}
...
<tr class="{$sequence}">
    <td><img src="{$language.locale|flag_icon}" alt="{$language.locale}" />&nbsp;<a href={concat( '/state/group/', $state_group.id , '/', $language.locale )|ezurl} title="{'View translation.'|i18n( 'design/admin/node/view/full' )}">{$language.name|wash}</td>
...
<tr class="{$sequence}">
    <td><input type="checkbox" name="RemoveIDList[]" value="{$state.id}" title="{'Select content object state group for removal.'|i18n( 'design/admin/state/group' )|wash}" {if $state_group.is_internal}disabled="disabled"{/if} /></td>
    ...
    <td>
        <input type="text" name="Order[{$state.id}]" size="3" value="{$state.priority}" {if $state_group.is_internal}disabled="disabled"{/if} />
    </td>
    <td>
    {if $state_group.is_internal|not}
