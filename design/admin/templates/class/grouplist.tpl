...
<tr class="{$Groups.sequence}">
    ...
    {* Remove. *}
    <td><input type="checkbox" name="DeleteIDArray[]" value="{$Groups.item.id}" title="{'Mark the <%class_group_name> class group for removal.'|i18n( 'design/admin/class/grouplist',, hash( '%class_group_name', $Groups.item.name ) )|wash}" /></td>
    ...
    {* Edit. *}
    <td><a href={concat( $module.functions.groupedit.uri, '/', $Groups.item.id )|ezurl}><img class="button" src={'edit.png'|ezimage} width="16" height="16" alt="{'Edit'|i18n( 'design/admin/class/grouplist' )}" title="{'Edit the <%class_group_name> class group.'|i18n( 'design/admin/class/grouplist',, hash( '%class_group_name', $Groups.item.name ) )|wash}" /></a></td>
...
<div class="block">
    ...
    <input class="button" type="submit" name="NewGroupButton" value="{'New class group'|i18n( 'design/admin/class/grouplist' )}" title="{'Create a new class group into which you can put classes.'|i18n( 'design/admin/class/grouplist' )}" />
</div>
...
{section show=$latest_classes}
    ...
    <tr>
        <th>{'Name'|i18n( 'design/admin/class/grouplist')}</th>
        <th>{'ID'|i18n( 'design/admin/class/grouplist' )}</th>
    ...
    {section var=LatestClasses loop=$latest_classes sequence=array( bglight, bgdark )}
        <tr class="{$LatestClasses.sequence}">
            ...
            {* Edit. *}
            <td><a href={concat( 'class/edit/', $LatestClasses.item.id )|ezurl}><img class="button" src={'edit.png'|ezimage} width="16" height="16" alt="{'Edit'|i18n( 'design/admin/class/grouplist' )}" title="{'Edit the <%class_name> class.'|i18n( 'design/admin/class/grouplist',, hash( '%class_name', $LatestClasses.item.name) )|wash}" /></a></td>
