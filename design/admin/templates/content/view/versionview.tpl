...
<h1 class="context-title"><a href={concat( '/class/view/', $object.contentclass_id )|ezurl} onclick="ezpopmenu_showTopLevel( event, 'ClassMenu', ez_createAArray( new Array( '%classID%', {$object.contentclass_id},'%objectID%',{$object.id},'%nodeID%',{$node.node_id},'%currentURL%','{**REMOVE**}{$node.url|wash( javascript )}{**/REMOVE**}{**ADD**{escape_javascript $node->url}**/ADD**}')), '{**REMOVE**}{$object.content_class.name|wash(javascript)}{**/REMOVE**}{**ADD**{escape_javascript $object->content_class->name}**/ADD**}', ['class-createnodefeed', 'class-removenodefeed'] ); return false;">{$object.content_class.identifier|class_icon( normal, $node.class_name )}</a>&nbsp;{$object.name|wash}&nbsp;[{$object.content_class.name|wash}]</h1>
