...
<table class="list-thumbnails" cellspacing="0">
    ...
    {let quoted_child=concat( '&quot;', $Nodes.item.name|wash(), '&quot;' )
        ...
        {section show=$Nodes.item.can_remove}
            ...
            {section-else}
            <input type="checkbox" name="DeleteIDArray[]" value="{$Nodes.item.node_id}" title="{'You do not have permissions to mark this item for removal.'|i18n( 'design/node/view/full' )}" disabled="disabled" />
        {/section}
