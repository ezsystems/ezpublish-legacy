...
<ul>
    {section var=Bookmarks loop=$bookmark_list}
       ...
       <a href="#" onclick="ezpopmenu_showTopLevel( event, 'BookmarkMenu', ez_createAArray( new Array( '%nodeID%', '{$Bookmarks.item.node_id}', '%objectID%', '{$Bookmarks.item.contentobject_id}', '%bookmarkID%', '{$Bookmarks.item.id}', '%languages%', {$Bookmarks.item.node.object.language_js_array} ) ) , '{**REMOVE**}{$Bookmarks.item.name|shorten(18)|wash(javascript)}{**/REMOVE**}{**ADD**{escape_javascript shorten($Bookmarks->name, 18)}**/ADD**}'); return false;">{$Bookmarks.item.node.object.content_class.identifier|class_icon( small, '[%classname] Click on the icon to display a context-sensitive menu.'|i18n( 'design/admin/pagelayout',, hash( '%classname', $Bookmarks.item.node.object.content_class.name  ) ) )}</a>&nbsp;<a href={$Bookmarks.item.node.url_alias|ezurl}>{$Bookmarks.item.node.name|wash}</a></li>
       {else}
