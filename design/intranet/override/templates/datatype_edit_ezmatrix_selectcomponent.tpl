...
{* included from datatype_edit_ezmatrix.tpl *} 
    <table>
    <tr>
    <th>
            ...
            {section var=column loop=$row.item.columns}
                ...
                {case match=0}
                    ...
                    {section var=selectbox_element loop=$:selectbox_content}
                        {section show=and(eq( first_set( $column.item|wash(xhtml),$default_matrix_field) , $selectbox_element.index), eq($selected_is_set, 0))}
                            <option selected value="{$selectbox_element.index}">{$selectbox_element.item}</option>
			    {set default_matrix_field=$default_matrix_field|inc }
			    {set selected_is_set=1}
                        {section-else}
                            <option value="{$selectbox_element.index}">{$selectbox_element.item}</option>
                        {/section}
                    {/section} 
                    </select>
                {/case}
