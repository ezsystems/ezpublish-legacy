...
class MonthOverview 
{
    public static function getOverview($input, $fieldGrouping, $dateTimestamp, $optional = array())
    {
        ...
        if ( !is_array( $input ) )
        {
            throw new ezcTemplateRuntimeException("Input is not an array");
        }
        ...
        foreach ( $input as $item )
        {
            ...
            {
                if ( $item->hasAttribute( $fieldGrouping ) )
                    $value = $item->attribute( $fieldGrouping );
            }
            else if ( is_array( $item ) )
            {
                if ( array_key_exists( $fieldGrouping, $item ) )
                    $value = $item[$fieldGrouping];
            }
            if ( $value !== null )
            {
                $info = getdate( $value );
