<h2>{"MySQL"|i18n("design/standard/setup/db")}</h2>
...
<p>{"MySQL is a good choice for handling most western languages, however it's currently not the best choice for Unicode or non-western languages."|i18n("design/standard/setup/db")}</p>

<h3>{"Installation"|i18n("design/standard/setup/db")}</h3>
<p>{"By using the"|i18n("design/standard/setup/db")} <tt class="option">--with-mysql</tt> {"configuration option you enable PHP to access MySQL databases. If you use this option without specifying the path to MySQL, PHP will use the built-in MySQL client libraries."|i18n("design/standard/setup/db")}</p>

<p>{"More information on the MySQL extension can be found at"|i18n("design/standard/setup/db")} <a href="http://www.php.net/manual/en/ref.mysql.php">php.net</a>.</p>
