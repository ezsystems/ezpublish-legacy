...
function eZSetupStep( &$tpl, &$http, &$ini, &$persistenceList )
{
    ...
    if ( $http->hasPostVariable( 'eZSetupDatabaseServer' ) );
        $databaseServer = $http->postVariable( 'eZSetupDatabaseServer' );
    if ( $http->hasPostVariable( 'eZSetupDatabaseName' ) );
        $databaseName = $http->postVariable( 'eZSetupDatabaseName' );
    if ( $http->hasPostVariable( 'eZSetupDatabaseUser' ) );
        $databaseUser = $http->postVariable( 'eZSetupDatabaseUser' );
    ...
    $persistenceList['database_info']['type'] = $databaseType;
    $persistenceList['database_info']['server'] = $databaseServer;
    $persistenceList['database_info']['name'] = $databaseName;
    $persistenceList['database_info']['user'] = $databaseUser;
