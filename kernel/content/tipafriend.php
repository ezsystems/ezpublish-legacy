...
$subject = ezpI18n::translate( 'kernel/content', 'Tip from %1: %2', null, array( $hostName, $nodeName ) );
$comment = '';
...
if ( $http->hasPostVariable( 'SendButton' ) )
{
    ...
    if ( !eZMail::validate( $yourEmail ) )
        $error_strings[] = ezpI18n::translate( 'kernel/content', 'The email address of the sender is not valid' );
    if ( !eZMail::validate( $receiversEmail ) )
        $error_strings[] = ezpI18n::translate( 'kernel/content', 'The email address of the receiver is not valid' );
    ...
    if ( $http->hasSessionVariable('ezpContentTipafriendList') )
    {
        if ( strpos( $http->sessionVariable('ezpContentTipafriendList'), $NodeID . '|' . $receiversEmail ) !== false )
            $error_strings[] = ezpI18n::translate( 'kernel/content', "You have already sent a tipafriend mail to this reciver regarding '$nodeName' content" );
    }

    if ( !isset( $error_strings[0] ) && !eZTipafriendRequest::checkReceiver( $receiversEmail ) )
        $error_strings[] = ezpI18n::translate( 'kernel/content', 'The receiver has already received the maximum number of tipafriend mails the last hours' );
...
$Result['path'] = array( array( 'text' => ezpI18n::translate( 'kernel/content', 'Tip a friend' ),
                                'url' => false ) );
