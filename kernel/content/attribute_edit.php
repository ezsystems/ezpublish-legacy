...
if ( $_SERVER['REQUEST_METHOD'] === 'POST' )
{
        ...
        count( $_POST ) === 0 )                                 // Therefore we also check if request got no post variables.
    {
        $validation['attributes'][] = array( 'id' => '1',
                                             ...
                                             'name' => eZi18n::translate( 'kernel/content', 'Error' ),
                                             'description' => eZi18n::translate( 'kernel/content', 'The request sent to the server was too big to be accepted. This probably means that you uploaded a file which was too big. The maximum allowed request size is %max_size_string.', null, array( '%max_size_string' => "$postMaxSize $postMaxSizeUnit" ) ) );
...
if ( $Module->isCurrentAction( 'Publish' ) )
{
    ...
    foreach ( array_keys( $assignments ) as $key )
    {
        ...
        {
            $validation[ 'placement' ][] = array( 'text' => eZi18n::translate( 'kernel/content', 'A node in the node assignment list has been deleted.' ) );
            $validation[ 'processed' ] = true;
    ...
    if ( !$mainFound and count( $assignments ) > 0 )
    {
        ...
        {
            $validation[ 'placement' ][] = array( 'text' => eZi18n::translate( 'kernel/content', 'No main node selected, please select one.' ) );
            $validation[ 'processed' ] = true;
            $inputValidated = false;
            eZDebugSetting::writeDebug( 'kernel-content-edit', "placement is not validated" );
        }
...
// $Result['path'] = array( array( 'text' => eZi18n::translate( 'kernel/content', 'Content' ),
//                                 'url' => false ),
//                          array( 'text' => eZi18n::translate( 'kernel/content', 'Edit' ),
//                                 'url' => false ),
