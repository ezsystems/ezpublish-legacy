    ...
    if ( is_object( $class ) )
    {
        ...
        return array( 'content' => $tpl->fetch( 'design:class/edit_locked.tpl' ),
                      'path' => array( array( 'url' => '/class/grouplist/',
                                              'text' => eZi18n::translate( 'kernel/class', 'Class list' ) ) ) );
    }
...
else
{
    ...
    if ( is_numeric( $GroupID ) and is_string( $GroupName ) and $GroupName != '' )
    {
        ...
        $class->setName( eZi18n::translate( 'kernel/class/edit', 'New Class' ), $EditLanguage );
        $class->store();
    ...
    foreach( $attributes as $attribute )
    {
        ...
        else
        {
            ...
            $datatypeValidation['attributes'][] =
                array( 'reason' => array( 'text' => eZi18n::translate( 'kernel/class', 'Could not load datatype: ' ).
                                           $attribute->attribute( 'data_type_string' )."\n".
                                           eZi18n::translate( 'kernel/class', 'Editing this content class may cause data corruption in your system.' ).'<br>'.
                                           eZi18n::translate( 'kernel/class', 'Press "Cancel" to safely exit this operation.').'<br>'.
                                           eZi18n::translate( 'kernel/class', 'Please contact your eZ Publish administrator to solve this problem.').'<br>' ),
            ...
            for ( $attrIndex2 = $attrIndex + 1; $attrIndex2 < count( $attributes ); $attrIndex2++ )
            {
                ...
                if (  $placement ==  $placement2 )
                {
                    $validation['attributes'][] = array( 'identifier' => $identifier2,
                                                         ...
                                                         'reason' => array ( 'text' => eZi18n::translate( 'kernel/class', 'duplicate attribute placement' ) ) );
                    $canStore = false;
                    break;
                }
                ...
                if ( $identifier == $identifier2 )
                {
                    $validation['attributes'][] = array( 'identifier' => $identifier,
                                                         ...
                                                         'reason' => array ( 'text' => eZi18n::translate( 'kernel/class', 'duplicate attribute identifier' ) ) );
                    $canStore = false;
                    break;
                }
            }
...
if ( $http->hasPostVariable( 'StoreButton' ) && $canStore )
{
    ...
    if( trim( $className ) == '' )
    {
        $validation['class_errors'][] = array( 'text' => eZi18n::translate( 'kernel/class', 'The class should have nonempty \'Name\' attribute.' ) );
    ...
    {
        $validation['class_errors'][] = array( 'text' => eZi18n::translate( 'kernel/class', 'The class should have at least one attribute.' ) );
        $basicClassPropertiesValid = false;
    }
    ...
    {
        $validation['class_errors'][] = array( 'text' => eZi18n::translate( 'kernel/class', 'There is a class already having the same identifier.' ) );
        $basicClassPropertiesValid = false;
    }
    unset( $classList );
...
if ( $http->hasPostVariable( 'NewButton' ) )
{
    ...
    $newAttribute->setName( eZi18n::translate( 'kernel/class/edit', 'new attribute' ) . $attrcnt, $EditLanguage );
