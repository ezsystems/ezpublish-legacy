...
if ( $http->hasPostVariable( "AddOrderStatusButton" ) )
{
    ...
    $messages[] = array( 'description' => eZi18n::translate( 'kernel/shop', 'New order status was successfully added.' ) );
}

if ( $http->hasPostVariable( "SaveOrderStatusButton" ) )
{
    $messages[] = array( 'description' => eZi18n::translate( 'kernel/shop', 'Changes to order status were successfully stored.' ) );
}
...
if ( $http->hasPostVariable( "RemoveOrderStatusButton" ) )
{
    ...
    if ( $hasRemoved )
        $messages[] = array( 'description' => eZi18n::translate( 'kernel/shop', 'Selected order statuses were successfully removed.' ) );
    if ( $triedRemoveInternal )
        $messages[] = array( 'description' => eZi18n::translate( 'kernel/shop', 'Internal orders cannot be removed.' ) );
}
...
$path = array();
$path[] = array( 'text' => eZi18n::translate( 'kernel/shop', 'Order list' ),
                 'url' => 'shop/orderlist' );
$path[] = array( 'text' => eZi18n::translate( 'kernel/shop', 'Status' ),
                 'url' => false );
