...
function applyChanges( $module, $http, $vatTypeArray = false )
{
    ...
    foreach ( $vatTypeArray as $vatType )
    {
        ...
        {
            if ( !$name )
                $errors[] = ezpI18n::translate( 'kernel/shop/vattype', 'Empty VAT type names are not allowed (corrected).' );
            else
                $errors[] = ezpI18n::translate( 'kernel/shop/vattype', 'Wrong VAT percentage (corrected).' );
...
{
    $commonPart = ezpI18n::translate( 'kernel/shop', 'VAT type' );
    $maxNumber = 0;
...
elseif ( $module->isCurrentAction( 'Remove' ) )
{
    ...
    if ( $haveDeps )
    {
        ...
        if ( ( count( $allVatTypes ) - count( $vatIDsToRemove ) ) == 0 )
        {
            ...
            $errors[] = ezpI18n::translate( 'kernel/shop/vattype', $errorMsg );
            $showDeps = false;
        }
        ...
        $path = array( array( 'text' => ezpI18n::translate( 'kernel/shop', 'VAT types' ),
                              'url'  => false ) );
...
$path = array();
$path[] = array( 'text' => ezpI18n::translate( 'kernel/shop', 'VAT types' ),
                 'url' => false );
