...
class eZISBN13
{
    ...
    */
    function extractISBNNumber( $isbnNr = false, &$error )
    {
        ...
        if ( is_numeric( $ean ) and strlen( $ean ) == self::LENGTH )
        {
            ...
            if ( $prefix == self::PREFIX_978 )
            {
                ...
                if ( $groupLength )
                {
                    ...
                    if ( $group instanceof eZISBNGroup )
                    {
                        ...
                        else
                        {
                            ...
                            {
                                $error = ezpI18n::translate( 'kernel/classes/datatypes', 'The registrant element of the ISBN number does not exist.' );
                                return false;
                            }
                        }
                    }
                    else
                    {
                        ...
                        if ( $strictValidation == 'true' )
                        {
                            $error = ezpI18n::translate( 'kernel/classes/datatypes', 'The ISBN number has a incorrect registration group number.' );
                            return false;
                        }
                    }
                }
                else
                {
                    ...
                    if ( $strictValidation == 'true' )
                    {
                        $error = ezpI18n::translate( 'kernel/classes/datatypes', 'The group element of the ISBN number does not exist.' );
                        return false;
                    }
                }
            }
            else
            {
                ...
                if ( $strictValidation == 'true' )
                {
                    $error = ezpI18n::translate( 'kernel/classes/datatypes', '%1 is not a valid prefix of the ISBN number.', null, array( $prefix ) );
                    return false;
                }
            }
        }
        ...
        {
            $error = ezpI18n::translate( 'kernel/classes/datatypes', 'All ISBN 13 characters need to be numeric' );
            return false;
        }
        return true;
    }
    ...
    */
    function validateISBN13Checksum ( $isbnNr, &$error )
    {
        ...
        {
            $error = ezpI18n::translate( 'kernel/classes/datatypes',
        ...
        {
            $error = ezpI18n::translate( 'kernel/classes/datatypes', 'ISBN length is invalid' );
            return false;
        }
        ...
        if ( ( $checksum13 % 10 ) != 0 )
        {
            ...
            //bad checksum
            $error = ezpI18n::translate( 'kernel/classes/datatypes', 'Bad checksum, last digit should be %1', null, array( $checkDigit ) );
            return false;
        }
