                    ...
                    switch( $httpType )
                    {
                        case eZURLOperator::HTTP_OPERATOR_TYPE_POST:
                        {
                            if ( $http->hasPostVariable( $httpName ) )
                                $operatorValue = !$checkExistence ? filter_var($http->postVariable( $httpName ), FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW|FILTER_FLAG_NO_ENCODE_QUOTES) : true;
                            else
                            {
                        ...
                        case eZURLOperator::HTTP_OPERATOR_TYPE_GET:
                        {
                            if ( $http->hasGetVariable( $httpName ) )
                                $operatorValue = !$checkExistence ? filter_var($http->getVariable( $httpName ), FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW|FILTER_FLAG_NO_ENCODE_QUOTES) : true;
                            else
                            {
                                if ( $checkExistence )
                                {
                        ...
                        case eZURLOperator::HTTP_OPERATOR_TYPE_COOKIE:
                        {
                            if ( array_key_exists( $httpName, $_COOKIE ) )
                                $operatorValue = !$checkExistence ? filter_var($_COOKIE[$httpName], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW|FILTER_FLAG_NO_ENCODE_QUOTES) : true;
                            else
                            {
                                if ( $checkExistence )
                                {
